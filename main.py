...
class AngleCorrectorApp:
    def __init__(self, root):
        ...
        # Set CustomTkinter to use system theme
        ...
        # Configure main frame grid with 4 columns (2/4 for tables, 1/4 for canvas, 1/4 empty)
        self.main_frame.grid_columnconfigure(0, weight=0)  # Input frame (fixed)
        self.main_frame.grid_columnconfigure(1, weight=2)  # Tables (2/4)
        self.main_frame.grid_columnconfigure(2, weight=1)  # Canvas (1/4)
        self.main_frame.grid_columnconfigure(3, weight=1)  # Empty (1/4)
        self.main_frame.grid_rowconfigure(0, weight=1)
        
        # Left frame for inputs (fixed width)
        ...
        # Right frame for output (split into tables and canvas)
        self.output_frame = ctk.CTkFrame(self.main_frame, corner_radius=10)
        self.output_frame.grid(row=0, column=1, columnspan=3, sticky="nsew", pady=10)
        
        # Tables frame (2/4 width)
        self.tables_frame = ctk.CTkFrame(self.output_frame, corner_radius=10)
        self.tables_frame.grid(row=0, column=0, sticky="nsew", padx=10)
        self.tables_frame.grid_columnconfigure(0, weight=1)
        
        # Canvas frame (1/4 width)
        self.canvas_frame = ctk.CTkFrame(self.output_frame, corner_radius=10)
        self.canvas_frame.grid(row=0, column=1, sticky="nsew", padx=(10, 0))
        
        # Input frame content
        ctk.CTkLabel(self.input_frame, text="Angle Corrector", font=("Helvetica", 18, "bold")).grid(row=0, column=0, columnspan=2, pady=10)
        ctk.CTkLabel(self.input_frame, text="Enter Angles (e.g., 235:20:25 or 235 20 25):", font=("Helvetica", 18, "italic")).grid(row=1, column=0, columnspan=2, pady=5)
        ...
        # Buttons
        ctk.CTkButton(self.input_frame, text="Add Field", command=self.add_angle_field, corner_radius=8, font=("Helvetica", 18)).grid(row=998, column=0, padx=5, pady=5, sticky="ew")
        ctk.CTkButton(self.input_frame, text="Clear Fields", command=self.clear_fields, corner_radius=8, font=("Helvetica", 18)).grid(row=998, column=1, padx=5, pady=5, sticky="ew")
        ctk.CTkButton(self.input_frame, text="View Results", command=self.correct_angles, corner_radius=8, font=("Helvetica", 18)).grid(row=999, column=0, columnspan=2, padx=5, pady=10, sticky="ew")
        
        # Tables frame content
        ctk.CTkLabel(self.tables_frame, text="Results", font=("Helvetica", 18, "bold")).pack(pady=10)
        
        # Raw angles table
        ctk.CTkLabel(self.tables_frame, text="Raw Angles:", font=("Helvetica", 18, "bold")).pack()
        self.raw_table_frame = ctk.CTkFrame(self.tables_frame, fg_color="#f0f0f0", corner_radius=5)
        ...
        for col, header in enumerate(headers):
            ctk.CTkLabel(self.raw_table_frame, text=header, font=("Helvetica", 18, "bold"), anchor="center", fg_color="#d9d9d9", corner_radius=5).grid(row=0, column=col, padx=1, pady=2, sticky="nsew")
        self.raw_table_labels = []
        
        # Error label
        self.error_label = ctk.CTkLabel(self.tables_frame, text="Error: 0 seconds", font=("Helvetica", 18, "bold"))
        self.error_label.pack(pady=5)
        
        # Corrected angles table
        ctk.CTkLabel(self.tables_frame, text="Corrected Angles:", font=("Helvetica", 18, "bold")).pack()
        self.corrected_table_frame = ctk.CTkFrame(self.tables_frame, fg_color="#f0f0f0", corner_radius=5)
        self.corrected_table_frame.pack(fill="x", padx=10, pady=5)
        for col in range(4):
            self.corrected_table_frame.grid_columnconfigure(col, weight=1)
        for col, header in enumerate(headers):
            ctk.CTkLabel(self.corrected_table_frame, text=header, font=("Helvetica", 18, "bold"), anchor="center", fg_color="#d9d9d9", corner_radius=5).grid(row=0, column=col, padx=1, pady=2, sticky="nsew")
        self.corrected_table_labels = []
        
        # Sum label
        self.sum_label = ctk.CTkLabel(self.tables_frame, text="Sum of Corrected Angles: 0° 0' 0\"", font=("Helvetica", 18, "bold"))
        self.sum_label.pack(pady=5)
        
        # Canvas frame content
        ctk.CTkLabel(self.canvas_frame, text="Polygon Visualization:", font=("Helvetica", 18, "bold")).pack()
        self.zoom_level = 1.0
        self.canvas = ctk.CTkCanvas(self.canvas_frame, bg="white", width=175, height=350, highlightthickness=1, highlightcolor="#007bff")  # Reduced width to 1/4
        ...
        ctk.CTkButton(self.zoom_frame, text="Zoom In", command=self.zoom_in, corner_radius=8, font=("Helvetica", 18)).pack(side="left", padx=5)
        ctk.CTkButton(self.zoom_frame, text="Zoom Out", command=self.zoom_out, corner_radius=8, font=("Helvetica", 18)).pack(side="left", padx=5)
        ctk.CTkButton(self.zoom_frame, text="Download Image", command=self.download_image, corner_radius=8, font=("Helvetica", 18)).pack(side="left", padx=5)
        
        # Status bar
        self.status_var = ctk.StringVar(value="Ready")
        self.status_bar = ctk.CTkLabel(self.root, textvariable=self.status_var, font=("Helvetica", 18, "bold"), anchor="w")
        self.status_bar.pack(side="bottom", fill="x")
    
    def add_angle_field(self):
        """Add a new angle input field in the grid."""
        index = len(self.angle_entries) + 1
        ctk.CTkLabel(self.input_frame, text=f"Angle {index}:", font=("Helvetica", 18, "bold")).grid(row=self.angle_row, column=0, padx=5, pady=2, sticky="w")
        entry = ctk.CTkEntry(self.input_frame, width=150, font=("Helvetica", 18))
    ...
    def draw_polygon(self, angles):
        ...
        side_length = 40  # Reduced for smaller canvas
        center_x, center_y = 87.5, 175  # Adjusted for 1/4 width
        vertices = []
        current_angle = 0
        ...
        scale_factor = min(175 / poly_width, 350 / poly_height) * 0.8 * self.zoom_level
        ...
        self.canvas.create_rectangle(0, 0, 175, 350, fill="white", outline="#007bff")
        ...
        for i, (x, y) in enumerate(scaled_vertices[:-1]):
            ...
            offset_x = 10 if x < center_x else -10
            offset_y = 10 if y < center_y else -10
            self.canvas.create_text(x + offset_x, y + offset_y, text=angle_text, font=("Helvetica", 12), fill="#333")
    ...
    def download_image(self):
        ...
        try:
            ...
            img = Image.new("RGB", (175, 350), "white")
            ...
            draw.rectangle([0, 0, 175, 350], fill="white", outline="#007bff")
            ...
            timestamp = "20250604_1731"  # 05:31 PM GMT, June 04, 2025
    ...
    def correct_angles(self):
        ...
        try:
            ...
            for i, (d, m, s) in enumerate(angles, 1):
                row_labels = []
                row_labels.append(ctk.CTkLabel(self.raw_table_frame, text=f"Angle {i}", anchor="center", font=("Helvetica", 18, "bold"), fg_color="#ffffff", corner_radius=5))
                row_labels[-1].grid(row=i, column=0, padx=1, pady=2, sticky="nsew")
                row_labels.append(ctk.CTkLabel(self.raw_table_frame, text=str(d), anchor="center", font=("Helvetica", 18, "bold"), fg_color="#ffffff", corner_radius=5))
                row_labels[-1].grid(row=i, column=1, padx=1, pady=2, sticky="nsew")
                row_labels.append(ctk.CTkLabel(self.raw_table_frame, text=str(m), anchor="center", font=("Helvetica", 18, "bold"), fg_color="#ffffff", corner_radius=5))
                row_labels[-1].grid(row=i, column=2, padx=1, pady=2, sticky="nsew")
                row_labels.append(ctk.CTkLabel(self.raw_table_frame, text=str(s), anchor="center", font=("Helvetica", 18, "bold"), fg_color="#ffffff", corner_radius=5))
            ...
            for i, (d, m, s) in enumerate(corrected_angles, 1):
                row_labels = []
                row_labels.append(ctk.CTkLabel(self.corrected_table_frame, text=f"Angle {i}", anchor="center", font=("Helvetica", 18, "bold"), fg_color="#ffffff", corner_radius=5))
                row_labels[-1].grid(row=i, column=0, padx=1, pady=2, sticky="nsew")
                row_labels.append(ctk.CTkLabel(self.corrected_table_frame, text=str(d), anchor="center", font=("Helvetica", 18, "bold"), fg_color="#ffffff", corner_radius=5))
                row_labels[-1].grid(row=i, column=1, padx=1, pady=2, sticky="nsew")
                row_labels.append(ctk.CTkLabel(self.corrected_table_frame, text=str(m), anchor="center", font=("Helvetica", 18, "bold"), fg_color="#ffffff", corner_radius=5))
                row_labels[-1].grid(row=i, column=2, padx=1, pady=2, sticky="nsew")
                row_labels.append(ctk.CTkLabel(self.corrected_table_frame, text=str(s), anchor="center", font=("Helvetica", 18, "bold"), fg_color="#ffffff", corner_radius=5))
                row_labels[-1].grid(row=i, column=3, padx=1, pady=2, sticky="nsew")
